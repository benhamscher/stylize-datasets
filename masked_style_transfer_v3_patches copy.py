...
def find_image_pairs(rgb_dir, mask_dir, pattern='*_*_leftImg8bit.png', panoptic_mask_suffix='_gtFine_panoptic.png', semantic_mask_suffix='_gtFine_labelTrainIds.png'):
    ...
    panoptic_paths = list(Path(mask_dir).rglob(f"*{panoptic_mask_suffix}"))
    random.shuffle(panoptic_paths)
    image_pairs = {}

    for rgb_path in rgb_paths:
        panoptic_mask_path = panoptic_paths.pop()
        base_name = panoptic_mask_path.stem.replace('_leftImg8bit', '')
        city_name = panoptic_mask_path.parent.name
...
def main():
    ...
    output_dirs = ['/home/bhamscher/datasets/Cityscapes_cue_/leftImg8bit_cue_conflict/train', 
                ...
                try:
                    ...
                    for id in unique_ids:
                        ...
                        if semantic_class is not None:
                            ...
                            # style_tensor = content_tf(style_img)

                            # # Set stylized_output as the unchanged style_tensor but unsqueezed on device
                            # stylized_output = style_tensor.unsqueeze(0).to(device)
                            ...
                            # stylized_out_filename = f"{content_name}_texture_{semantic_class}{content_path.suffix}"
                            # stylized_output_name = out_dir.joinpath(stylized_out_filename)
                            # # Save the stylized image
                            # save_image(stylized_output.cpu(), stylized_output_name, padding=0)
