...
# Add at start of file after imports
def cleanup_gpu():
...
def find_image_pairs(rgb_dir, mask_dir, pattern='*_*_leftImg8bit.png', panoptic_mask_suffix='_gtFine_panoptic.png', semantic_mask_suffix='_gtFine_labelTrainIds.png'):
    ...
    for rgb_path in rgb_paths:
        ...
        split_name = rgb_path.parent.parent.name
        panoptic_mask_filename = f"{base_name}{panoptic_mask_suffix}"
        semantic_mask_filename = f"{base_name}{semantic_mask_suffix}"
        panoptic_mask_path = Path(mask_dir) / split_name / city_name / panoptic_mask_filename
        semantic_mask_path = Path(mask_dir) / split_name / city_name / semantic_mask_filename
...
def main():
    ...
    parser.add_argument('--mode', type=str, default='PC', choices=['PC', 'CS'],
                        help='Dataset type, implemented for PASCAL Context (PC) and Cityscapes (CS)')
    ...
    for content_dir, output_dir, mask_dir in zip(content_dirs, output_dirs, mask_dirs):
        ...
        if args.txt_path is not None:
            ...
            if args.mode == 'CS':
                ...
                content_images = [Path(img) for img in image_pairs.keys()]
            elif args.mode == 'PC':
                content_images = find_images(content_dir, mask_dir)
                print(f'Found {len(content_images)} content images')
            else: # Try PC mode
                print("Invalid mode. Defaulting to PASCAL Context.")
                content_images = find_images(content_dir, mask_dir)
                print(f'Found {len(content_images)} content images')
